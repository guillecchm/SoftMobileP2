 ...
 * menos un (@see {@link edu.uclm.esi.iso2.banco20193capas.model.Cliente}) que
 * sea titular.
 *
 *
 */
...
public class Cuenta {
    ...
    public Cuenta(final Long identification) {
        this();
        this.id = identification;
    }

    public Cuenta(final Integer identification) {
        this(Long.valueOf(identification));
    }
     ...
     * @throws CuentaYaCreadaException Si la cuenta ya se ha almacenado en la
     *         base de datos
     */
    ...
    private void ingresar(final double importe, final String concepto)
        ...
        MovimientoCuenta movimiento = new MovimientoCuenta(this, importe,
                concepto);
    ...
    /**
     * Retirar un importe de la cuenta bancaria.
     * @param importe Cantidad a retirar
     * @param concepto Justificación
     * @throws ImporteInvalidoException Si el importe es menor o igual que 0
     * @throws SaldoInsuficienteException Si el importe es mayor que getSaldo()
     */
    public void retirar(final double importe, final String concepto)
            throws ImporteInvalidoException, SaldoInsuficienteException {
        if (importe <= 0) {
            throw new ImporteInvalidoException(importe);
        }
        ...
        final MovimientoCuenta movimiento = new MovimientoCuenta(this, -importe,
                concepto);
        Manager.getMovimientoDAO().save(movimiento);
    }
     ...
     * primer parámetro. Se cobra una comisión del 1%, con un mínimo de 1.5
     * euros.
     *
     ...
     *         existe en la BD
     * @throws ImporteInvalidoException Si el importe es es menor o igual que 0
     * @throws SaldoInsuficienteException Si la cuenta no tiene saldo suficiente
     *         para afrontar el importe y la comisión
     */
    public void transferir(final Long numeroCuentaDestino, final double importe,
            final String concepto) throws CuentaInvalidaException,
    ...
    private Cuenta load(final Long numero) throws CuentaInvalidaException {
        final Optional<Cuenta> optCuenta = Manager.getCuentaDAO()
                .findById(numero);
    ...
    public final double getSaldo() {
        final List<MovimientoCuenta> mov_cuenta = Manager.getMovimientoDAO()
                .findByCuentaId(this.id);
        double saldo = 0.0;
        for (final MovimientoCuenta movimiento : mov_cuenta) {
     ...
     *
     * @throws CuentaSinTitularesException Si no se ha asignado ningún titular a
     *         la cuenta
     */
     ...
     * @param identification el identificador.
     */
     ...
     * @return titulares
     */
    public final List<Cliente> getTitulares() {
        return titulares;
    }

    public final void setTitulares(final List<Cliente> titulares) {
        this.titulares = titulares;
    }

    /**
     * Retorna si una cuenta ha sido creada.
     * @return creada.
     */
    public final boolean isCreada() {
        return creada;
    }

    public final void setCreada(final boolean creada) {
        this.creada = creada;
    }
}
